...
internal class TempDrawerPal
{
    ...
    private bool TryExtractPart(PixelLine sourceLine, ref int position, [NotNullWhen((true))] out PartLine? result)
    {
        ...
        for ( ; position < sourceLine.Length; position++)
        {
            ...
            if (current.IsVisible)
            {
                ...
                if (foreground != currentBackground)
                    break;
    ...
    public TempDrawerPal(Color defaultBackground, Color defaultForeground,  bool borderConflictsAllowed)
    {
